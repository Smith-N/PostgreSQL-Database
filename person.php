...
{
  $id = $_GET['id'];
  $query = "SELECT fName, lName, phone, address, zip, gradYear, category, corpName FROM person WHERE donorid='$id'";
  $result = pg_query($query);
...
if($_GET['save'])
{
  ...
  $query = "UPDATE person SET fName='$fName', lName='$lName', phone=$phone, address='$address', zip=$zip, gradYear=$gradYear, category='$category', corpName='$corpName' WHERE donorid='$id'";
  pg_query($query);
}
